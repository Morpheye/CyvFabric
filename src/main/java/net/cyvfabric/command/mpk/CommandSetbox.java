...
public class CommandSetbox extends CyvCommand {
    ...
    public void run(CommandContext<FabricClientCommandSource> context, String[] args) {
        try {
            AABB player = Minecraft.getInstance().player.getBoundingBox();

            double x1 = Double.parseDouble(args[0]) + player.getXsize()/2;
            double x2 = Double.parseDouble(args[1]) - player.getXsize()/2;
            double y1 = Double.parseDouble(args[2]);
            double y2 = Double.parseDouble(args[3]);
            double z1 = Double.parseDouble(args[4]) + player.getZsize()/2;
            double z2 = Double.parseDouble(args[5]) - player.getZsize()/2;
            AABB box = new AABB(Math.min(x1, x2), Math.min(y1, y2), Math.min(z1, z2),
                    Math.max(x1, x2), Math.max(y1, y2), Math.max(z1, z2));

            if (ParkourTickListener.landingBlock != null) {
                ParkourTickListener.landingBlock.bb = new AABB[]{box};
